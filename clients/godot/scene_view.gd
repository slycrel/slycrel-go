...
func _draw() -> void:
	...
	# HUD (to the right of the grid)
	var hud = current_scene.get("hud", {})
	if typeof(hud) != TYPE_DICTIONARY or hud.is_empty():
		return

	var hud_x := cols * CELL_W + HUD_PAD
	var y := 20.0
	var step := HUD_LINE_H

	_hud_line(Vector2(hud_x, y), str(hud.get("char_name", "")), HUD_COLOR_NAME); y += step
	_hud_line(Vector2(hud_x, y), "HP: %d/%d" % [int(hud.get("hp", 0)), int(hud.get("max_hp", 0))], HUD_COLOR_STAT); y += step
	_hud_line(Vector2(hud_x, y), "Mv: %d" % int(hud.get("mv", 0)), HUD_COLOR_STAT); y += step
	_hud_line(Vector2(hud_x, y), "Ps: %d/%d" % [int(hud.get("ps", 0)), int(hud.get("max_ps", 0))], HUD_COLOR_STAT); y += step * 2

	var w1 := str(hud.get("w1", ""))
	if w1 == "": w1 = "Hands"
	_hud_line(Vector2(hud_x, y), "W1: " + w1, HUD_COLOR_GEAR); y += step

	var w2 := str(hud.get("w2", ""))
	if w2 == "": w2 = "---"
	_hud_line(Vector2(hud_x, y), "W2: " + w2, HUD_COLOR_GEAR); y += step

	var ar := str(hud.get("ar", ""))
	if ar == "": ar = "None"
	_hud_line(Vector2(hud_x, y), "Ar: " + ar, HUD_COLOR_GEAR); y += step * 2

	var mname := str(hud.get("monster_name", ""))
	if mname != "":
		_hud_line(Vector2(hud_x, y), mname, HUD_COLOR_ENEMY); y += step
		_hud_line(Vector2(hud_x, y), "HP: %d" % int(hud.get("monster_hp", 0)), HUD_COLOR_ENEMY); y += step * 2

	var log_lines = hud.get("log", [])
	if typeof(log_lines) == TYPE_ARRAY:
		for line in log_lines:
			_hud_line(Vector2(hud_x, y), str(line), HUD_COLOR_LOG)
			y += step
